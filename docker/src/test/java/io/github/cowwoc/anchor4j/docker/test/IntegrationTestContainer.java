package io.github.cowwoc.anchor4j.docker.test;
...
public class IntegrationTestContainer
{
		...
		boolean ignored) throws IOException, InterruptedException, TimeoutException
	{
		...
		if (!suffix.isEmpty())
			name += "." + suffix;
		this.name = name;
		this.client = (InternalDocker) Docker.connect();
		...
		if (client.getImageState(DOCKER_IN_DOCKER) == null)
		{
	...
	private static int getHostPort(ContainerState container) throws IOException, InterruptedException
	{
		List<ContainerState.PortBinding> boundPorts = container.getNetworkConfiguration().ports();
		for (ContainerState.PortBinding port : boundPorts)
		{
