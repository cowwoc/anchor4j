...
public interface SwarmCreator
{
	 ...
	 */
	WelcomePackage create() throws IOException, InterruptedException;
	 ...
	 */
	record WelcomePackage(String managerId, JoinToken workerJoinToken)
	{
		/**
		 * @param managerId       the ID of the manager that created the swarm
		 * @param workerJoinToken the secret value needed to join the swarm as a worker
		...
		{
			requireThat(managerId, "nodeId").doesNotContainWhitespace().isNotEmpty();
